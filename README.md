...
site/
  ...
  profile.html    every /u/* path                                 router.js
    ...
    embed.js      the generator: charts, banners, badges, versus cards
                  and Steam artwork to take away
...
| `/u/<profile>/embed` | the generator: a bar chart, a badge, a banner, a card of this profile against another, a full-size Steam artwork, or a Unicode chart of this library - each one as an address that stays current or as a file that does not, and with the figures on it chosen from one menu. Steam's own About Me only loads pictures already on Steam's hosts, so the page says so and offers the download for that case. The artwork also takes a picture of the reader's own, composited in the browser and never uploaded, which is why that one is download-only |
